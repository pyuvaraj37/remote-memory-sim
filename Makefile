...
VPP := v++
VPP_PFLAGS := 
CMD_ARGS = $(BUILD_DIR)/${KERNEL}_krnl.xclbin
SDCARD := sd_card
...
HOST_SRCS += ./src/${KERNEL}_host.cpp 
...
EXECUTABLE = ./${KERNEL}_host
EMCONFIG_DIR = $(TEMP_DIR)
EMU_DIR = $(SDCARD)/data/emulation

############################## Declaring Binary Containers ##############################
BINARY_CONTAINERS += $(BUILD_DIR)/${KERNEL}_krnl.xclbin
...
$(BUILD_DIR)/${KERNEL}_krnl.xclbin: $(BINARY_CONTAINER_krnl_OBJS)
	mkdir -p $(BUILD_DIR)
ifeq ($(HOST_ARCH), x86)
	$(VPP) $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(TEMP_DIR) -o'$(BUILD_DIR)/${KERNEL}_krnl.link.xclbin' $(+)
	$(VPP) -p $(BUILD_DIR)/${KERNEL}_krnl.link.xclbin -t $(TARGET) --platform $(PLATFORM) --package.out_dir $(PACKAGE_OUT) -o $(BUILD_DIR)/${KERNEL}_krnl.xclbin
else
	$(VPP) $(VPP_FLAGS) -l $(VPP_LDFLAGS) --temp_dir $(TEMP_DIR) -o'$(BUILD_DIR)/${KERNEL}_krnl.xclbin' $(+)
endif
