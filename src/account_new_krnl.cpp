...
void bank(
    ...
    */
    ...
    //std::cout << "Starting account accelerator..." << std::endl; 
    RUBIS_MAIN_LOOP: while (debug_counter < debug_exe && counter < number_of_operations || !done) {
    //while (counter < number_of_operations) {
        ...
        switch (operation_list[counter])
        {
            ...
            case 0: {
                ...
                if (!proposed.full() && done) {
                    temp_amount = amount_list[counter];
                    if (bank_accounts + remote_bank_accounts - temp_amount >= 0 && temp_amount != 0) {
        ...
        if (!smr_updated.empty()) {
            ...
            {
            case 0:
                std::cout << "Withdraw Amount: " << local_update.range(29, 0) << std::endl; 
                bank_accounts -= local_update.range(31, 0);
                break;
        ...
        if (!update_rsp.empty()) {
            ...
            {
            case 0:
                std::cout << "Withdraw Amount: " << remote_update.range(29, 0) << std::endl; 
                bank_accounts -= remote_update.range(31, 0);
                break;
...
extern "C" void account_new_krnl(
    hls::stream<pkt256>& m_axis_tx_meta,
    hls::stream<pkt64>& m_axis_tx_data,
