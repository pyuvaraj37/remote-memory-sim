...
void project(
    ...
    */
    ...
    static ap_uint<16> failCounter; 

    static bool employee[2000];
    static bool projects[2000];
    //HASHMAP for PROJECTS TABLE 1
    htUpdateResp<KEY_SIZE,VALUE_SIZE> response;
    htLookupResp<KEY_SIZE,VALUE_SIZE> lookupResponse; 
    VITIS_LOOP_543_1: for (int i = 0; i < 1000; i++) {
        employee[i] = true;
        projects[i] = true;
        response = insert<KEY_SIZE,VALUE_SIZE>(htUpdateReq<KEY_SIZE,VALUE_SIZE>(KV_INSERT, i, 1, 0), failCounter);
        if (!response.success) {
            std::cout << 'FAILED - Tried to insert at key: ' << i << std::endl; 
        }
    }
    ...
    //while (counter < number_of_operations) {
        ...
        if (!permissibility_rsp.empty() && !proposed.full()) {
            ...
            std::cout << "KERNEL: Permissibility Check" << std::endl; 
            switch(permiss_rsp.range(31, 30)) {
                ...
                */
                case 0: {
                    ...
                    if (lookupResponse.value == 0 && !lookupResponse.hit) {
        ...
        if (!smr_updated.empty()) {
            std::cout << "Local Update" << std::endl; 
            ap_uint<256> temp;
            smr_updated.read(temp);
            update = temp.range(31, 0);
            done = true;
            counter++;
            ...
            case 0:
                ...
                response = insert<KEY_SIZE,VALUE_SIZE>(htUpdateReq<KEY_SIZE,VALUE_SIZE>(KV_INSERT, update.range(29, 0), 1, 0), failCounter);
                project_size++;
                break;
            
            case 1:
                std::cout << "DeleteProject - Project ID: " << update.range(29, 0) << std::endl; 
                response = remove<KEY_SIZE,VALUE_SIZE>(htUpdateReq<KEY_SIZE,VALUE_SIZE>(KV_INSERT, update.range(29, 0), 1, 0));
