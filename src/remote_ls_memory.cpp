#include "remote_ls_memory.h"

#define DEBUG_MEMORY 0
#define LEADER_SWITCH 1
...
/*
    ...
    Performs Remote Leader Switch by incrementing HBs. 

*/
extern "C" void remote_ls_memory(hls::stream<ap_uint<256>>& s_axis_tx_meta, hls::stream<ap_uint<64>>& s_axis_tx_range, volatile int* HBM_PTR) {
    ...
    #if LEADER_SWITCH
    //Increment HB at remote node
    for (int i = 0; i < MAX_NUMBER_NODES-1; i++) {
        #if DEBUG_MEMORY
        std::cout << "HB INC " << " at " << i << " - " << i+1 << std::endl; 
        #endif
        BRAM_REMOTE_MEMORY[i][i+1] += 1; 
    }
    #endif
